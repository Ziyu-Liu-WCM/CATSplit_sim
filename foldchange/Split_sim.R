...
# define TP genera
# spiked_genera <- c("g__Faecalibacterium", "g__Veillonella", "g__Lactobacillus",
...
# added <- c("g__Sutterella", "g__Odoribacter","g__Bilophila", "g__Sutterella","g__Alistipes", "g__Dysgonomonas", "g__Dialister")

### high abundance set
spiked_genera <- c("g__Faecalibacterium", "g__Collinsella", "g__Bacteroides", "g__Prevotella", 
                   "g__Lactobacillus", "g__Ruminococcus", "g__Veillonella", "g__Holdemania", "g__Fusobacterium",
                   "g__Escherichia", "g__Akkermansia", "g__Sutterella", "g__Odoribacter","g__Bilophila", 
                   "g__Sutterella","g__Alistipes", "g__Dysgonomonas", "g__Dialister")
...
if(!file.exists(sim_data_file)){
  sim_data <- foreach(seedNum=1:200,
                      .packages=c("ape","GUniFrac","vegan","dirmult")) %dopar% {
                        generate_sim(seedNum, testTaxon, FoldChange, taxonomy_table, otu_table, tree_data, averageCount, marginal, sampleSize, scalefac)
                      }
...
##
selected_features <- CATSplit_parallel(sim_data[[8]], taxonomy_table, meta_table, tree_data, metric, parallel, nCore, nReps = 50)
...
# 
# a <- genus_count[rownames(genus_count) %in% setdiff(spiked_chain_genera,selected_features_1),]
# 
# highprev <- genus_count[rowSums(genus_count > 0) > ncol(genus_count)*0.1,] 
# sorted <- sort(apply(highprev, 1, mean), decreasing = TRUE)
# 
# sorted_names <- names(sorted[1:50]) 
# sorted_names[! sorted_names %in% selected_features_1]
# view(selected_features_1)
